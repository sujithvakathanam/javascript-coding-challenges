...
function hasHiddenFee (prices, t) {
  const sum = prices.reduce(
    (acc, cv) => acc + Number(cv.replace('$', '').replace('"', '')
      .trim())
    , 0
  );
  console.log('sum is', sum);
  console.log('number is', Number(t.replace('$', '').replace('"', '')
    .trim()));
...
const hasHiddenFee1 = (prices, t) => prices.reduce((acc, cur) => acc + Number(cur.slice(1)), 0) < Number(t.slice(1));
