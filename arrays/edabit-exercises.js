...
const arr = ['John', 'Jay', 'Vikram', 'John', 'Sindbad', 'Vikram'];
...
const array = [5, 8, 2, 1, 3, 4, 7];

const missingNo = function (array) {
  const sortedArray = array.sort();
  console.log('sorted  arreay is ', sortedArray);
  const retArray = sortedArray.filter((item, index) => sortedArray.indexOf(item) + 1 === index + 1);
...
// Solution 3:
const sevenBoom2 = arr => (/7/.test(arr) ? 'Boom!' : 'there is no 7 in the array');
