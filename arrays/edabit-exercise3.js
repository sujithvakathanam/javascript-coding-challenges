...
function XO (str) {
  ...
  str = str.toLowerCase();
  const countOfX = str.match(/x/gi) == null ? 0 : str.match(/x/g).length;
  const countOfO = str.match(/o/gi) == null ? 0 : str.match(/o/g).length;
...
// Solution 3:
function XO2 (str) {
  const check = str.replace(/[^x]/ig, '');
  console.log('check is', check);

  const check1 = str.replace(/[^x]/ig, '');
