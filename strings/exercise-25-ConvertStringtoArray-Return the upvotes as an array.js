...
function transformUpvotes(str) {
    let newArr =[];
    str.split(' ').map((item) =>item.endsWith('k') ? newArr.push(+(item.slice(0,item.length-1)) * 1000) : newArr.push(item));
    return newArr;
}
...
function transformUpvotes1(str) {
    return  str.split(' ').map((item) =>item.endsWith('k') ? +(item.slice(0,item.length-1)) * 1000 : +item);
}
