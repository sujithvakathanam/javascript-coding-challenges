...
function multiplicity (arr) {
  ...
  for (let i = 0; i < uniqueArray.length; i++) {
    finalArray.push([uniqueArray[i], obj[uniqueArray[i]]]);
  }
...
console.log(multiplicity([5, 5, 1, 1, 5, 5, 3]));

/*
 * Solution 2: Using map()
 *  Please refer this url -https://javascript.info/map-set
 *  Map is a collection of keyed data items, just like an Object. But the main difference is that Map allows keys of any type.
 */
const multiplicity1 = arr => Array.from(arr.reduce((map, item) => map.set(item, map.get(item) + 1 || 1), new Map()));

console.log(multiplicity1([5, 5, 1, 1, 5, 5, 3]));
