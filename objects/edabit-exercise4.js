...
function findOccurrences(str, char) {
  ...
  let reg = new RegExp(char.toLowerCase(),"gi"); // flag - g represents global, i represents search everywhere. ie, dont stop at the first match

  newStr.map((item) =>{
      if(!map[item]) {
          let count = item.match(reg) == null ? 0 : item.match(reg).length;
          map[item] = count;
      }
